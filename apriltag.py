...
class Apriltag(object):
    ...
    def create_detector(self, family='tag16h5', sigma=0.8, nthread=1, debug=False, minarea=400,thresholding = 'adaptive',downsampling = False):
        '''
...
#            img = cv2.GaussianBlur(img, (7, 7), self._quad_sigma)
        ...
        """
        if (self._thresholding == 'canny'):
            contours, hierarchy = cv2.findContours(img, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)  #检测所有的轮廓  外层是顶层
        else:
            contours, hierarchy = cv2.findContours(img, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)  # 只检测外围的轮廓  找角点RETR_EXTERNAL
        ##################################
        if self._debug:
        ...
        for i in range(len(contours)):
            if (hierarchy[0, i, 3] < 0 and contours[i].shape[0] >= 4):
                ...
                if area > self._minarea:
                    hull = cv2.convexHull(contours[i])  # 凸包逼近
                    if (area / cv2.contourArea(hull) > 0.8):
                        if (self._debug):
                            hulls.append(hull)
                        quad = cv2.approxPolyDP(hull, 8, True)  # 多边形逼近maximum_area_inscribed
                        if (len(quad) == 4):
